...
for PROJECT_DIR in "../frontend" "../webcomponent"; do
  PROJECT="$SCRIPT_DIR/$PROJECT_DIR"
  ...
  if [ ! -f "$CFG" ]; then
    echo "Config file $CFG not found - continue"
    continue
  fi
  ...
  if [ -z "${VER:-}" ]; then
    echo " No valid version found - continue"
    continue
  fi
  ...
  # Locale desired filename and location
  LOCAL_JAR="$VER.jar"
